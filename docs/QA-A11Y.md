...
1. ~~`fidelity.guard_tree()` calls only `check_pair` and `check_copy`~~ **CLOSED, verified 2026-09-23:** both L7 (`ungraded_surfaces`) and L8 (`check_kit`) are folded into `guard_tree`, with the swapped-kit failure recorded in the comment beside them. Original text: `guard_tree()` calls only `check_pair` and `check_copy`. L7 (`ungraded_surfaces`) and L8 (`check_kit`) are reachable only from the CLI. Re-verified this pass at `tests/a11y/fidelity.py:456-481`. A swapped host kit flipped the one credential failure green with `guard_tree` returning 0 findings. **Fold both into `guard_tree`, or delete `guard_tree` and make the CLI the only entry point.** Owner: `tests/a11y` (fidelity lane). Three lines.
2. ~~Every self-oracled check needs a second, literal assertion~~ **CLOSED 2026-09-23.** `RULED` in `check_bus.py` holds the literal strings from docs/UX.md 7.1 and M2-03 8.1, and `check_ruled` fails three ways: the bus disagreeing with the table, the composer disagreeing with the table, or both. Six `-LIT` checks added beside the six self-oracled ones. **Proven on the exact mutations this document records as invisible:** the bar saying `nothing on` instead of `IPTV, idle`, the bar saying `tv broke` instead of `IPTV, playlist error`, and the thousands separator removed from the count each took the run from 3 failures to 5 -- the `-LIT` check and the new cardinality check, both red, where the measurement recorded on line 285 saw zero new failures. Owner: harness lane.
3. ~~Assert name and value on the same node, plus cardinality~~ **CLOSED 2026-09-23.** `pick()` returned a list and every caller tested it for truthiness, so two nodes carrying one name passed exactly as one did -- and a reader hears a duplicated label as two things to choose between. `exactly_one()` and `check_one()` assert role, name, cardinality and, where the table rules one, the Value, on ONE node. Five added: `L2-BAR-04-{idle,playing,error}` (the ruled bar name on exactly one button), `L2-Q-06`, `L2-SC-05`, `L2-SC-06`. Owner: harness lane.
...
6. ~~`TRANSFORM_RULES` is constrained by nothing~~ **CLOSED 2026-09-23.** Two poisons added, and the first draft of one exposed why the gap was real. A rule that merely fails to fire is caught by **L2** as a stale rule, and L1 is never reached -- so a table poisoned IN STEP with the copy would pass the weaker layer. The test now edits the generated copy so the table and the copy agree with each other and only **L1** disagrees with both: a rule declaring an edit to an `Accessible.` line as legitimate. Disabling L1 turns it red, along with two existing checks. A second poison pins the wildcard direction: a rule whose before-block matches many lines is refused by `apply_transform`, so a table entry cannot be widened into one that quietly covers edits nobody declared. 32 -> 34 tests. Owner: fidelity lane.
...
9. ~~no mode that mutates a shipping file and regenerates~~ **CLOSED 2026-09-23** by `tests/a11y/mutate_shipping.py`, and it found what it was predicted to find. Mutating `Guide.qml` itself and regenerating the tree from the mutated source: dropping an `Accessible.name` takes the declarations from 84 to 82, dropping an `Accessible.role` likewise, turning a role into `Accessible.ignored` takes 84 to 83 -- and the guard reports **0 findings** for all three. That is the shared oracle, exposed exactly as item 9 said it would be: the guard compares a generated copy against the source it was generated FROM, so a source that loses a declaration loses it on both sides at once and there is nothing left to compare. It is not a defect in the guard, it is the boundary of what the guard answers, and the runner prints that boundary rather than a bare count. CONSEQUENCE, which is the reason this mattered: **a green fidelity run is evidence that the copy is faithful and is not evidence that the guide's accessibility is intact**, and no filed run may be read as though it were. One process note worth keeping: the first version of this runner executed the fidelity SUITE against the mutated repo and reported three mutations as caught. They were not. Several of that suite's cases anchor on text in the shipping guide, so mutating the text broke the FIXTURES and the redness had nothing to do with detection. Asking `check_pair` directly is what produced the real answer. Owner: harness lane.
10. ~~`mutationProof` lines for the three `Model.js` tree mutations omit `L2-FIDELITY`~~ **CLOSED 2026-09-23, by running them rather than by correcting a list.** A list a human is expected to keep in step with a suite is the rule 13 failure this project keeps paying for, so `mutate_bus.py` was run and its own output is the record: `L2-FIDELITY` is in the newly-red set of **every** mutation that edits a file inside the tree -- all four `Model.js` cases and all fourteen `GuideProbe.qml` ones -- and all 23 mutations are killed. One honest gap the run exposes: the two `BarWidget.qml` mutations do NOT turn `L2-FIDELITY` red, because that file's sha256 is checked by `make_hosts.py` at build time and nothing re-checks it during the run. Both are still killed by 8 and 11 checks respectively, so it is a gap in the fidelity LAYER's coverage and not in detection. Owner: harness lane.
