...
| Cursor | The selected plate takes the selection fill and border, AND carries the cursor mark of the 2026-09-21 ruling. The caption stays in the foreground, cursor or not, exactly as the row's name does: 5.4 reserves the calibrated accent for ACTIVE and forbids it on a cursor with no exceptions, and the first build of this tile broke that. The mark is not optional here: the selection fill is 8 per cent alpha, which reads on a 52 px row and disappears over a 230 px tile. Measured by looking at the first build of this view. |
...
**This amends 3.2's arrow rows**, and 3.2 says so in place -- search mode
routes the arrows through its own handler, which is what the 0.8.0 preflight
blocked on. 3.1 is the LIST-mode table and its movement rows are written for
the list; the wall's list-mode keys are specified here, in this paragraph, and
nowhere else. (An earlier version of this sentence claimed to amend 3.1 too
and did not, so the two tables stated opposite things about the same keys.) All of it is one table, `Model.arrowAction`, which the
