...
| Its claim | Measured |
|---|---|
| "Four open rungs" in the contrast family | One open question, three bookkeeping actions. D-RUNG-5's fix shipped at `62e7d39` and the row still reads `open`; D-RUNG-3's row describes `Model.BAR_IDLE_ALPHA` at factor 1.25 and floor 4.71, a symbol that exists in no source file - the shipping code is `BarWidget.qml:72`, `Util.alpha(root.barFg, Model.BAR_IDLE_ALPHA)` at 0.86. Verified this pass by grep and by reading both files |
