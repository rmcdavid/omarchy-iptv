...
| A dispatch step exits non-zero after the fallback | Stop the queue, log one redacted warning, leave the window as it is, report `dispatch_failed`. A half-applied PiP is visible and the next `p` reads the live state and finishes or undoes it |
