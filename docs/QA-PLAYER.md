...
| PLY-FAIL-12 | the mark clears when the channel plays again | H PLY-H02 | a successful play of a previously failed channel removes it from `failedAt` (`Model.withoutFailed`); the row loses U+F0026 and the `Failed HH:MM` detail; **[amended 2026-09-24]** the mark now PERSISTS in that source's cache; it is dropped when the channel plays, when it ages out, when its id leaves the playlist, and when the source is removed |
...
3. **PO-2's escape hatch is not a runnable command.** The README tells the
   ...
   form, and add it to Uninstall. **Filed as D-REL-3 (2026-09-25); see the board.**
