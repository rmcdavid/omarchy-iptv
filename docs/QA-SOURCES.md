...
- "How" column: `A` = automated (exact command and file), `H` = harness
  ...
  live shell (section 9), `X` = observed at the real accessibility sink by the
  accessibility harness (`tests/a11y/`), a lane tool that is deliberately not
  in `scripts/check.sh` (PLAN-NEXT decision 9). A case may list several. `A*` =
  an automated test the lanes are expected to add per ARCH 8.3; until it lands
  QA runs the command by hand. An `X` case is satisfied only by a dated run
  whose output is filed in `docs/QA-RESULTS.md` under "Accessibility harness
  runs", never by the harness merely existing.
- **A criterion may not be a grep for the string the implementation was written
  to contain** (CLAUDE.md rule 14). SRC-A11Y-01 and SRC-A11Y-05 were exactly
  that. SRC-A11Y-05 was the worst of the four on this project: it claimed to
  prove that a screen reader never gets a secret, its How column was the single
  word `grep`, and there was a secret reaching that sink the whole time it
  passed (D-A11Y-1). Both are rewritten as `X` cases below.
...
| ID | Verifies | How | Expected |
|---|---|---|---|
| SRC-A11Y-01 | roles and names (7.1) | X Sources and form scenarios of the accessibility harness, dated run filed in `docs/QA-RESULTS.md`. **Not** a grep: the old criterion was `grep -n 'Accessible.' Guide.qml`, which matches the 56 lines the file was written to contain and cannot go red (rule 14) | Per row of UX-SOURCES 7.1, the node is present in the walked AT-SPI tree with the stated role and name: Heading (titles), Button `Sources, 3 saved`, List `Sources`, ListItem `<label>, <host>, <n> channels in <m> groups` + `, active` + `, EPG` + `, last used <x>` / `, never used` / `, not loaded yet` with `focused` = cursor and `selected` = active, ListItem action rows, Button `Edit <label>` / `Remove <label>`, Dialog `Add source` / `Edit source` / `Add Xtream login` / `Set up a playlist`, EditableText `Label, optional` / `Playlist URL or path` / `EPG URL, optional` / `Server URL` / `Username` / `Password`, Button eye `Show query` / `Hide query` with `checked`, Button link rows `Use Xtream login instead` / `Saved sources, 3`, Button `Load` / `Save` / `Cancel`, AlertMessage result line, Dialog confirm with its two Buttons. A row of UX-SOURCES 7.1 that no scenario populates carries UNVERIFIED there and is not claimed here. **Markup only.** Delivery of any of it to an assistive technology is UNVERIFIED and impossible today (D-GS-3) |
...
| SRC-A11Y-05 | the **unconsented** credential sinks (6.7, PLAN-NEXT decision 7) | X Xtream and form scenarios of the accessibility harness, dated run filed in `docs/QA-RESULTS.md`. **Not** a grep, and not a property-presence check: the old How column was the word `grep`, it claimed to prove that no secret reaches a screen reader, and a secret was reaching that sink the entire time it passed (D-A11Y-1). The assertion reads the accessible **Value**, which is what Qt publishes from the control's own text and what a masked `Accessible.description` cannot protect | Two halves, **both required; the first alone is not a pass.** (a) *Absence.* With the Xtream form open and its fields populated by the scenario and by **no user action**, no node anywhere in the walked tree carries the server token, the username token or the password token in its name, its description or its value; the server and username fields are named `Server URL` and `Username`, and the password field carries its label through `Accessible.description` (**not** `passwordEdit`: deleted under PLAN-NEXT decision 5, inert in both directions on Qt 6.11.2). (b) *The field is undamaged.* What the user typed is still in the field, character for character, before and after the walk: digest, length and content equal. Half (b) exists because a proposed remedy for D-A11Y-1 fed the masked string back through `onTextChanged` into the stored value, and a check that asked only (a) reported it green -- a run that certifies silent credential replacement is a failed run, not a passing one. **Out of scope by ruling:** nothing is asserted about what a *deliberately revealed* field may publish. PLAN-NEXT decision 8 is unanswered, and an assertion written before it would encode a policy nobody has ruled on. **Markup only.** Delivery is UNVERIFIED (D-GS-3) |
...
| SRC-PRIV-07 | 6.7 the accessibility bus (a CLAUDE.md rule 5 sink) | X SRC-A11Y-05 | As SRC-A11Y-05: the unconsented Xtream server and username are asserted at the **Value**, the field is asserted undamaged, and the consented reveal is deliberately unasserted pending PLAN-NEXT decision 8. This row's old title, "accessibility never exposes the query", was false when it was written: a revealed URL field publishes its query in full (D-A11Y-1), and UX-SOURCES 6.7 item 7 now says so |
