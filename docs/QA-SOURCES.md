...
Owner: QA. Status: v0.1 (2026-09-13), written while Lane 1 (Model.js + Guide.qml)
and Lane 2 (Service.qml + helper + harness) build M2-01 in parallel, so the pass
can start the moment the lanes merge. Extends `docs/QA.md` (v0.2: ids, methods,
...
| SRC-FR-05 | probe failure keeps the setup state (S8) | H SRC-H05 | form thaws, result line U+F0026 `Connection refused from 127.0.0.1` (never the URL), focus back on `Playlist`, values kept; no `updateEntryInline` call, `barConfig` unchanged, `configured` stays false; whether a `fetchedAt: 0` record with a session error stays in history is SRC-DEC-13 |
...
| SRC-LST-08 | row order | H SRC-H06 | active first, then the rule of SRC-DEC-22 (ARCH assumes `lastUsed` desc; the UX wireframe does not state one); recorded, not judged, until the lead rules |
...
| SRC-SW-06 | never-fetched source (added by CLI, or cache deleted) | H SRC-H09 | Enter commits, then the shipped `Loading playlist...` / `Fetching from <host>` state; failure -> shipped error state whose hint reads `r retry - o sources - Esc close` (UX 5.3) so the previous source is two keys away |
...
| SRC-LBL-02 | derived duplicates | A (same); H SRC-H11 | second source on the same host gets the suffix of SRC-DEC-11 (` 2` or ` (2)`), third ` 3` / ` (3)`; comparison case-insensitive per UX 5.6 |
...
| SRC-LBL-06 | label length cap | A T13/T14; H SRC-H03 | 64 characters accepted, 65 -> `Label too long - max 64 characters`; the field's `maximumLength` is 64 so a longer paste is cut at the field boundary |
...
| ID | Verifies | How | Expected |
|---|---|---|---|
| SRC-CLI-01 | `omarchy bar set ... playlistUrl <new>` adds a record | H SRC-H09; L 9.3 | `ipc set playlistUrl http://127.0.0.1:8765/qa-src-b.m3u` with Sources open: a row appears without reopening, `origin: "cli"`, label derived, active, first fetch runs (no cache yet) then `5 channels` |
...
| SRC-XT-02 | URL building vectors | A `node` over `xtream-expect.json` V01-V14 (section 2); H SRC-H10 `xtream(...)` | `playlistUrl`, `epgUrl`, `host` and `key` exactly as the fixture; percent-encoding of ` `, `&`, `=`, `+`, `/`, `?`, `#`, `%`, `@`, `:`, `!*'()` and UTF-8; parameter order fixed; trailing slash and `get.php`/`xmltv.php`/`player_api.php`/`c` stripped once |
| SRC-XT-03 | validation errors | A E01-E09; H SRC-H10 | one error at a time in form order, focus moves: `Enter the server URL`, `Server must start with http:// or https://`, `Server is just http://host:port - no path`, `Invalid URL - check the host`, `Enter the username`, `Enter the password`; over-cap fields per SRC-DEC-08; schemeless server per SRC-DEC-06 |
...
| SRC-ERR-05 | the 51st source (`too_many`) | H SRC-H11 | `addSource` returns `too_many`; the user-facing message and whether `a` / the `Add source` row are disabled at the cap are SRC-DEC-14; `canAddSource` false |
...
| SRC-ERR-07 | persist failure (risk R2) | H SRC-H14 (optional harness verb `failPersist`) | `updateEntryInline` returning false while a change was needed -> `sourcesPersistFailed`; the guide's copy per SRC-DEC-15; the fallback argv `["omarchy","bar","put",id]` then `["omarchy","bar","set",id,"playlistUrl",url]` is logged by the harness as keys only and never executed against the real shell (the harness must stub `omarchy` on PATH or the verb must be off by default) |
...
| SRC-ERR-09 | slow probe ends on the helper deadline | H SRC-H14 (trickle server) | `Timed out from 127.0.0.1` after ~60 s (helper wall-clock deadline, S-05), form thaws; the 180 s service watchdog is the backstop (not exercised) |
...
|---|---|---|---|
| SRC-SEC-01 | clipboard content as data: control characters | H SRC-H04 (`paste/paste-multiline.txt`) | `wl-copy < file` then `Ctrl+V`: the field holds one line `http://127.0.0.1:8765/qa-src-a.m3u` (leading spaces and the trailing TAB trimmed/stripped, the second line gone); nothing runs; Enter probes the URL; the console never shows the pasted text |
| SRC-SEC-02 | 100 KB paste | H SRC-H04 (`python3 -c "print('http://h.test/' + 'a'*102400, end='')" \| wl-copy`) | the field holds 2,048 characters (`maximumLength`), the UI stays responsive (a screenshot within 1 s), validator `too_long` on Enter without a helper run, `state()` dump under 4 KB for the field, no 100 KB string in any log |
...
| SRC-SEC-08 | cache key path traversal in `cache remove` / `prune` | H SRC-H15; A* `tests/test_helper.py` | keys `../x`, `../../x`, `abc`, `d5977d8a/..`, `d5977d8a/../../x`, `%2e%2e`, `` (empty), `D5977D8A` (uppercase), `d5977d8a-1000` (suffix over 3 digits), `d5977d8a-0`: every one -> `{"ok":false,"kind":"cache","error":{"code":"bad_key","message":"invalid cache key"}}` exit 1 and a canary file placed next to `sources/` untouched; a symlink `sources/deadbeef -> $SCRATCH/canary` -> the realpath guard refuses (`bad_key` or `skipped`), the canary intact; `--keep` with a bad key -> `bad_key`; no argv path echoed in any message |
...
## 10. Decisions the lead must make before the pass (SRC-DEC)

Found while cross-reading UX-SOURCES.md, ARCHITECTURE-SOURCES.md and SR1-SR10
for this plan; none is settled by a ruling. The fixtures carry the ARCH
expectation where one exists and mark the record with the id.
